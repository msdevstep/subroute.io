...
namespace Subroute.Common
{
    ...
    /// allows crossing the AppDomain boundry so that ExecuteTrigger and ExecuteServiceAction methods may be called. Anything executed inside this method is safe
    ...
    public class ExecutionSandbox : MarshalByRefObject
    {
        public RouteResponse Execute(byte[] assemblyBytes, RouteRequest request)
        {
            ...
            // Void methods are treated as a no content result, so we'll create the ExecutionResponse object ourselves with NoContent status code.
